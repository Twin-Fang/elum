...
### AppShake — 잘못된 입력 알림 (Figma 명세 없음)

시안에 오류 연출이 없다. 이 서비스는 **경고색·경고 아이콘을 쓰지 않으므로**
(보호자와 아이가 함께 보는 화면에서 실패가 질책처럼 읽히면 안 된다) 움직임으로만 알린다.

| 항목 | 값 | 근거 |
|---|---|---|
| 지속 | `AppMotion.shake` 440ms | 2.5주기가 들어가 한 왕복 약 175ms. 더 짧으면 신경질적, 더 길면 늘어진다 |
| 진폭 | `AppMotion.shakeAmplitude` 10 | 지수 감쇠를 거쳐 실제 첫 스윙은 7px대 — 점 지름(20)의 3분의 1 남짓 |
| 감쇠 | λ = 3.5 | 스윙이 매번 정확히 절반으로 줄어든다 (7.2 → 3.6 → 1.8 → 0.9px) |
| 주기 | 2.5 | 0.5의 배수여야 시작과 끝이 모두 0이다. 어긋나면 어긋난 자리에 굳는다 |

등폭으로 같은 횟수를 흔드는 흔한 에러 shake는 기계가 튕기는 것처럼 보인다.
손으로 건드린 물체가 멎는 감쇠를 그대로 쓴다 — `A·e^(−λt)·sin(2π·c·t)`.

동작 줄이기가 켜져 있으면 흔들지 않고 햅틱만 남긴다. 적용처: PIN 재입력 불일치.
