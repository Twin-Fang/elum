...
| 상태 | 배경 | 텍스트 |
| --- | --- | --- |
| **disabled** | `#818393` | `rgba(255,255,255,0.5)` |
| **enabled** | `brandOrange` `#FF8B22` | `#FFFFFF` |

> ⚠️ Figma 온보딩 프레임은 전부 **disabled 상태**로 그려져 있다.
> enabled 색은 브랜드 컬러 기준으로 **추론한 값** — 디자이너 확인 필요. → [미확정](#미확정-사항)
...
- **버튼 enabled 상태 색상** — Figma에 disabled만 존재. 현재 `#FF8B22` 추론.
- **목표 칩 선택 상태(selected) 스타일** — `온보딩_목표_선택` 프레임에 있으나 색 토큰 미추출.
