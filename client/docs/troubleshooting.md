...
- [카드 진행률이 100%였다가 재진입하면 57%로 되돌아감](#카드-진행률이-100였다가-재진입하면-57로-되돌아감)
...
## 카드 진행률이 100%였다가 재진입하면 57%로 되돌아감

**언제**: 2026-09-08 · 이슈 #139
**증상**: 아동 모드에서 카드 7장을 전부 체크해 100%가 됐는데, 앱을 다시 열면 57%(4/7).
함께 "아동 모드에 두 번째 일과가 안 보인다"는 보고가 왔지만 그건 버그가 아니었다 —
보호자 승인 전(PENDING_REVIEW) 일과는 서버가 아동 화면에서 의도적으로 뺀다(docs 원칙 3번).

**원인**: **클라이언트와 서버의 완료 규칙이 달랐고, 서버의 거부를 클라이언트가 삼켰다.**

| | 서버 `RoutineService` | 클라이언트 (수정 전) |
| --- | --- | --- |
| 완료 | 이전 단계 미완료면 **409** | 아무 카드나 체크 가능 |
| 취소 | 마지막 완료 단계만 허용, 아니면 **409** | 아무 카드나 해제 가능 |
| 실패 응답 | — | `debugPrint`만 하고 **무시** |
| 동기화 | — | `unawaited`로 **동시 발사** (순서 보장 없음) |

운영 로그로 확인한 실제 순서: 4번 체크 → (더블탭) 4번 취소 → 5번 체크 **409** →
4번 재체크 → 6번 **409** → 7번 **409**. 서버엔 4/7만 남았고, 화면은 로컬 메모리
Set에 7장이 다 들어 있어 100%로 보였다. 로컬 상태는 메모리뿐이라 앱을 다시 열면 사라지고
서버 값이 드러난다.

**해결** (`child_routine_notifier.dart` · `step_progress_repository.dart`):

1. **화면이 서버와 같은 순서 규칙을 먼저 지킨다** — `canToggle()`. 앞 단계가 남았거나
   뒤 단계가 이미 체크된 카드는 눌러도 바뀌지 않고, 체크 버튼을 흐리게 보여준다
   (아동 화면이라 빨강·경고는 쓰지 않는다)
2. **repository가 성공 여부를 돌려준다** (`Future<bool>`). 서버가 거부하면 notifier가
   로컬 표시를 걷어내 서버 값으로 되돌린다
3. **동기화 요청을 직렬 체인으로 보낸다** — 앞 요청이 끝난 뒤에만 다음을 보낸다
4. 동기화가 끝나면 `todayRoutinesProvider`·`myRoutinesProvider`를 무효화해 목록이
   서버 진실을 다시 읽게 한다. 상세 화면도 스냅샷 대신 목록 provider에서 같은 id를 찾아 쓴다

**재발 방지**:

- **낙관적 UI를 쓰면 반드시 "거부되면 되돌린다"까지 한 쌍으로 만든다.** 실패를 삼키는
  repository는 화면이 거짓말하게 만든다. `StepProgressRepository`가 `bool`을 돌려주는
  이유가 이것이다 — 새 동기화 메서드도 같은 계약을 따른다.
- **서버가 상태 전이 규칙을 강제하면 클라이언트도 같은 규칙을 갖는다.** 규칙을 모르는
  화면이 자유롭게 상태를 바꾸면 서버 거부가 누적된다. 서버 규칙은
  `server/.../RoutineService.java`에서 직접 읽는다(추측 금지).
