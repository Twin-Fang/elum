...
> 출처: Figma 최상위 트리 (node `238:1846`) 하위 온보딩 프레임
...
온보딩 섹션 `238:3022`를 덤프하면 0~11번 프레임이 한 번에 나온다.

> ⚠️ 12번 `온보딩_맞춤설정완료`(`204:1042`)는 이름과 달리 **보호자 섹션(`309:2836`)에 있다.**
> 온보딩 섹션만 덤프하면 나오지 않으므로 별도로 조회한다.
...
## 온보딩 이후 화면 (2026-07-21 덤프에서 확인)

온보딩을 마치면 아래 화면으로 이어진다. 최상위 `238:1846` 아래에 별도 섹션으로 있다.

| 섹션·프레임 | 노드 ID | 내용 |
| --- | --- | --- |
| 보호자 (SECTION) | `309:2836` | 홈 · 새로운 일과 만들기(입력·로딩·추가질문·카드확인) · 맞춤설정완료 |
| 아이_홈 | `309:3172` | 아이용 홈 화면 |
| 보호자_아이화면_전환 | `309:2837` | PIN 입력 후 아이 화면으로 전환 |

`보호자` 섹션에는 프레임이 21개 있고 대부분 **같은 화면의 상태 변형**이다.
구현 전에 해당 섹션을 덤프해 어떤 것이 별도 화면이고 어떤 것이 상태인지 먼저 가른다.
...
- PIN 재입력 불일치 시 문구
- `보호자` 섹션 21개 프레임 중 실제 화면 수 (상태 변형과 미분류)
