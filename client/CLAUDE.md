...
| 문서 | 내용 |
| --- | --- |
| [docs/figma-frames.md](./docs/figma-frames.md) | **화면 지도** — 전체 프레임 노드 ID + 대조 현황. 화면을 건드리기 전에 |
...
최상위 아래 세 섹션에 프레임 45개가 들어 있다 (2026-09-21 덤프).

| 노드 | 이름 | 프레임 |
|---|---|---|
| `238:3022` | 온보딩_보호자 | 18 — 로그인·역할·이름·목표·캐릭터·비밀번호·코드연결 |
| `309:2836` | 보호자 | 17 — 홈·일과 시트·일과 만들기 전 과정 |
| `425:4392` | 아이 | 10 — 홈·화면 전환·별·보상(캐릭터 3종) |

**노드 ID 전체와 화면별 대조 현황은 [docs/figma-frames.md](./docs/figma-frames.md)에 있다.**
화면을 건드리기 전에 **그 화면이 대조에 올라가 있는지 먼저 본다.** 안 올라간 화면은
어긋나도 아무도 모른다 — 실제로 일과 시트에서 여섯 곳, 별 화면에서 21%가 그렇게 나왔다.
...
**출시 때도 `lib/core/dev/`를 지우지 않는다** (이슈 #220에서 결정).
코드는 바이너리에 남지만 `showDevTools=false`라 실행 경로가 없고, QA 토큰은
`release` 프로파일이 `.env`에서 지운다. **지키는 선은 코드가 아니라 값이다.**
