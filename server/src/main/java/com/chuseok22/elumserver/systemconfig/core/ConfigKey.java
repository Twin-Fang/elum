...
public enum ConfigKey {
  ...
  MEMBER_WITHDRAWN_RETENTION_DAYS(
    ...
    "탈퇴 뒤 계정 식별값(소셜 로그인 식별번호·서비스 아이디)·AI 이용 기록·비밀번호 변환값(아이디 가입)·약관 동의 기록을 남겨 두는 기간. 이 안에 같은 소셜 계정이나 같은 아이디와 비밀번호로 로그인하면 이전 계정을 빈 상태로 되살려 사용량을 잇고(약관 동의는 다시 받는다), 지나면 완전히 지운다. ⚠️ 개인정보처리방침 4조의 보관 기간(1년 = 365)과 같아야 한다 — 바꾸려면 방침을 먼저 개정하고 시행 7일 전에 알린다. 1~3650",
