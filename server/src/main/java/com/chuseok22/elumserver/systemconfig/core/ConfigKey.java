...
public enum ConfigKey {
  ...
  // 탈퇴 계정 보관 기간 (이슈 #372).
  //
  // 탈퇴해도 계정 식별값(소셜 로그인 식별자)과 AI 이용 기록을 이 기간만큼 남긴다. 같은 소셜 계정으로
  // 다시 가입해 무료 사용량을 0 부터 새로 받는 것을 막기 위해서다. 지나면 스케줄러가 완전히 지운다.
  // ⚠️ 365 는 임시값이다. 법 확인 뒤 확정하고, 개인정보처리방침에 적힌 기간과 반드시 같게 둔다.
  MEMBER_WITHDRAWN_RETENTION_DAYS(
    ConfigGroup.MEMBER, "탈퇴 계정 보관 일수",
    "탈퇴 뒤 계정 식별값과 AI 이용 기록을 남겨 두는 기간. 이 안에 같은 소셜 계정으로 오면 이전 계정을 빈 상태로 되살려 사용량을 잇고, 지나면 완전히 지운다. 개인정보처리방침의 보관 기간과 같게 둔다. 1~3650",
    ConfigValueType.INTEGER, List.of(), "365", 1, 3650
  ),
