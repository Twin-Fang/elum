...
public final class PromptDefaults {
  ...
  public static final Map<PromptKey, String> DEFAULTS = Map.of(
    ...
    PromptKey.GEMINI_ROUTINE_CREATE_PREFIX, """
      ...
      [description 작성 규칙]
      - 사용자에게 소리 내어 읽어주는 문장입니다. title과 같은 행동을 가리키되, 무엇을 \
      어떻게 하는지 조금 더 자세하게 한 문장으로 풀어씁니다.
      - 예: title이 "옷을 입어요"이면 description은 "학교에 입고 갈 옷을 차례대로 입어요"

      [문장 표현 규칙]
      - title과 description 모두 사용자에게 직접 말하듯 "~해요" 체를 사용합니다.
      ...
      - 마음 상태를 바꾸라는 단계를 만들지 않습니다. "마음을 다스려요", "진정해요", \
      "참아요", "기분을 가라앉혀요"처럼 속으로 하는 일은 수행 여부를 확인할 수 없어 \
      카드가 되지 못합니다.
      - routineText가 감정·기분에 관한 것이면(예: 속상할 때, 화날 때, 불안할 때), \
      그 상황에서 실제로 할 수 있는 관찰 가능한 행동으로 바꿉니다. \
      예: "마음을 다스려요"가 아니라 "조용한 곳으로 가요", "물을 한 컵 마셔요", \
      "숨을 크게 다섯 번 쉬어요".
      ...
      - INDEPENDENT: 당사자가 스스로 수행하는 행동 중심으로 씁니다.
    ...
    PromptKey.GEMINI_ROUTINE_IMAGE_PREFIX, """
      ...
      [캐릭터]
      - character.appearance에 적힌 생김새 그대로 그립니다. **그 묘사가 이 카드의 \
      주인공입니다.** 사람으로 바꾸거나 다른 동물로 바꾸지 않습니다.
      - 참조 이미지가 첨부되어 있으면(character.referenceImageProvided가 true) 그 그림을 \
      우선 따르고 appearance는 보조로 씁니다. 반드시 그 캐릭터와 동일한 얼굴 형태, 머리, \
      복장, 색상, 신체 비율로 그리며 새롭게 재해석하지 않습니다.
      - 다른 캐릭터를 임의로 추가하지 않습니다.
      - character 자체가 없을 때만 특정 캐릭터를 지정하지 않은, 나이를 짐작하기 어려운 \
      단순한 인물로 그립니다.
      ...
      [스타일]
      - 두꺼운 외곽선 없는 심플한 플랫 벡터, 파스텔 톤의 단색 위주 색상, 그림자와 질감은 \
      최소화합니다.
      - **배경은 흰색이나 아주 연한 단색 한 가지로 채웁니다.** 그라데이션이나 어두운 \
      배경, 사진 같은 배경은 쓰지 않습니다.
      - 참조 이미지가 첨부되어 있으면 그 화풍을 그대로 따릅니다.
      - 장면마다 동일한 스타일을 유지합니다.
