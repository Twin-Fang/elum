...
가능하다. 프롬프트 품질을 빠르게 반복 개선하는 핵심 도구다.
...
7. **보상(강화물)은 보호자가 설정하고, 앱은 그것을 보여주고 상기시키기만 한다.** 앱이 보상을 결정하지 않는다.
   앱 내 별(⭐)은 성취 연출이고, 강화물은 보호자가 정한 **현실의 보상**(간식·놀이·산책)이다 — 둘은 다른 것이다.

## 문서

초기 기획·설계 과정이 궁금하다면 01번부터 순서대로 볼 것을 추천한다.

> 자문·회의 기록은 [docs/meetings/](./docs/meetings/), 대회 일정·제출 요건은 [docs/hackathon/](./docs/hackathon/).
...
| [07-mvp-scope.md](./docs/07-mvp-scope.md) | MVP 구현 범위, 데모 시나리오 |
