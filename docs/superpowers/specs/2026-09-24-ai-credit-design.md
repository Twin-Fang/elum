...
| 재가입 | 장부를 유지한다. 1년 안은 계정 복원으로 이어지고, 보관 기간이 지나면 방침대로 식별값을 파기한다 | 사용자 확인 · 방침 4조 |
...
- 1년 보관 기간 안의 재가입은 기존 계정이 복원되므로 장부가 member_id 로 그대로 이어진다.
- purge(보관 기간 경과) 시 account.member_id 와 identity_key 를 함께 비운다(행·원장은 운영 지표로 유지). 방침 4조 "탈퇴일로부터 1년간 보관한 뒤 파기"와 맞춘다 — `WithdrawnRetentionMatchesPrivacyPolicyTest` 가 고정한다.
