...
디자인이 된다. 대신 첫 실행 시 기기 고유 ID로 계정을 자동 생성한다.

```
첫 실행:   기기ID 생성 → signup → login → 토큰 저장
이후 실행: 저장된 토큰 사용
401 발생:  저장된 자격증명으로 재login → 원요청 1회 재시도
```

보호자는 아이디·비밀번호의 존재를 알 필요가 없다.

서버 제약을 지켜야 한다. `username`은 4~20자라 UUID(36자)를 그대로 못 쓰고,
`password`는 8~64자다.

```dart
/// 기기별 자동 계정. 보호자는 아이디·비밀번호를 알 필요가 없다.
/// Figma에 로그인 화면이 없고, 온보딩 흐름을 끊지 않기 위한 선택이다.
/// username은 서버 제약이 4~20자라 UUID를 그대로 쓸 수 없다.
static String usernameFrom(String deviceId) =>
    'elum_${deviceId.replaceAll('-', '').substring(0, 12)}';
```
...
`LocalStorage`에 `accessToken` / `deviceId` / `password`를 추가한다.

**일과 원문은 여전히 저장하지 않는다** (docs 원칙 5번). 저장되는 것은 온보딩 결과
4개와 인증 정보 3개뿐이다.
