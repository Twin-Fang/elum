...
- 옵션 타입이 걸린 3개 계층은 서로 강하게 결합돼 있어(레코드 타입 변경 시 그 계층의 메서드 본문도
  함께 컴파일이 깨짐), Task를 "레코드 정의"와 "메서드 로직"으로 더 세분화할 수 없다. 각 Task는
  한 계층(레이어)의 레코드+로직+테스트를 통째로 바꾼다.
...
### Task 1: Gemini 질문 스키마/프롬프트/파이프라인 — emoji+label 구조 도입

**Files:**
- Modify: `server/src/main/java/com/chuseok22/elumserver/ai/core/RoutineQuestionDraft.java`
- Modify: `server/src/main/java/com/chuseok22/elumserver/ai/infrastructure/client/GeminiTextClient.java:179-200` (`questionResponseSchema()`)
- Modify: `server/src/main/java/com/chuseok22/elumserver/ai/core/PromptDefaults.java:97-103` (`GEMINI_ROUTINE_QUESTION_PREFIX` 값)
...
**Interfaces:**
- Produces: `RoutineQuestionDraft.QuestionItem.Option(String emoji, String label)` — Gemini JSON
  파싱 결과 타입(Task 1 내부에서만 소비됨)
- Produces: `RoutineAiPipeline.RoutineQuestionResult.QuestionResultItem.OptionResult(String emoji, String label)` —
  Task 2가 소비한다. `RoutineAiPipeline.generateQuestion(String nickname, Set<SupportGoal> supportGoals, String sanitizedInputText): RoutineQuestionResult`
  시그니처는 바뀌지 않는다.

이 Task는 레코드 정의를 바꾸는 즉시 그 레코드를 다루는 메서드 본문(`generateQuestion`,
`fallbackQuestion`)도 함께 컴파일이 깨지므로, 두 가지를 한 번에 바꾼다. 테스트도 같은 이유로
"기존 테스트를 새 구조로 갱신 + 새 동작에 대한 테스트 추가"를 한 스텝에서 함께 진행한다.
...
`server/src/main/java/com/chuseok22/elumserver/ai/infrastructure/client/GeminiTextClient.java`의
179~200번째 줄(`private Map<String, Object> questionResponseSchema() { ... }` 전체)을 아래로
교체한다.
...
`server/src/main/java/com/chuseok22/elumserver/ai/core/PromptDefaults.java`의 97~103번째 줄을
아래로 교체한다(바로 다음 줄인 104번째 줄의 `);`는 그대로 둔다).
...
- [ ] **Step 4: `RoutineAiPipeline.java`의 레코드 정의를 emoji/label 구조로 교체**

197~202번째 줄(`RoutineQuestionResult` record 전체)을 아래로 교체한다.
...
64~89번째 줄(`generateQuestion` 메서드 전체)을 아래로 교체한다.
...
92~107번째 줄(`fallbackQuestion` 메서드 전체)을 아래로 교체하고, 바로 아래에 짧은 헬퍼
메서드 `option(...)`을 추가한다(중첩 타입명이 길어 가독성을 위해 둔다).

```java
  // 선택한 도움 목표 각각에 대해 개별 질문을 만든다(여러 목표를 하나로 합치지 않음).
  // "직접 입력"은 보호자가 자유 텍스트를 입력하도록 유도하는 항목이라 추천 답변 목록에
  // 절대 포함하지 않는다(서비스 정책).
...
- [ ] **Step 7: `./gradlew compileJava`로 프로덕션 코드 컴파일 확인**

Run: `cd server && ./gradlew compileJava`
Expected: `BUILD SUCCESSFUL`

- [ ] **Step 8: `RoutineAiPipelineTest.java`의 import에 `tuple` 추가**

3번째 줄(`import static org.assertj.core.api.Assertions.assertThatThrownBy;`) 바로 아래에
한 줄을 추가한다.
...
- [ ] **Step 9: 기존 `generateQuestion_validResponse_returnsMappedQuestions` 테스트를 emoji/label 구조로 갱신**

52~67번째 줄(테스트 메서드 전체)을 아래로 교체한다.
...
- [ ] **Step 10: label 빈 옵션은 제외하고 나머지는 유지하는 테스트 추가**

Step 9로 교체한 메서드 바로 다음(원래 69번째 줄 위치)에 새 테스트를 추가한다.
...
- [ ] **Step 11: fallback 답변에 emoji가 채워지고 "직접 입력"이 없는지 검증하는 테스트 추가**

기존 `generateQuestion_geminiFails_fallsBackToGoalMappedQuestions` 테스트(현재 69~80번째 줄) 바로
다음에 새 테스트를 추가한다. 기존 테스트 자체는 그대로 둔다(질문 개수만 검증하던 테스트라 옵션
타입 변경에 영향받지 않음).
...
### Task 2: API 응답 계층 — `RoutineQuestionResponse`/`RoutineService`

**Files:**
- Modify: `server/src/main/java/com/chuseok22/elumserver/routine/application/dto/response/RoutineQuestionResponse.java`
- Modify: `server/src/main/java/com/chuseok22/elumserver/routine/application/service/RoutineService.java:68-70`
- Test: `server/src/test/java/com/chuseok22/elumserver/routine/application/service/RoutineServiceTest.java`

**Interfaces:**
- Consumes: `RoutineAiPipeline.RoutineQuestionResult.QuestionResultItem.OptionResult(String emoji, String label)` (Task 1 산출물)
- Produces: `RoutineQuestionResponse.QuestionItem.OptionItem(String emoji, String label)` — API 최종
  응답 형태. `RoutineService.generateQuestion(String memberId, RoutineQuestionRequest request): RoutineQuestionResponse`
  시그니처는 바뀌지 않는다.

- [ ] **Step 1: `RoutineQuestionResponse.java`를 emoji/label 구조로 전면 교체**
...
- [ ] **Step 2: `RoutineService.generateQuestion()`의 매핑 라인을 emoji/label 변환으로 교체**
...
`toOptionItems` 헬퍼는 `generateQuestion` 메서드(72번째 줄에서 끝남) 바로 다음, `create` 메서드
(원래 74번째 줄) 이전에 위치시킨다. 위 코드 블록은 `generateQuestion`의 마지막 두 줄 + 새 헬퍼
메서드 전체를 포함한다.

- [ ] **Step 3: `./gradlew compileJava`로 컴파일 확인**

Run: `cd server && ./gradlew compileJava`
Expected: `BUILD SUCCESSFUL`

- [ ] **Step 4: `RoutineServiceTest.java`에 `tuple` import 추가**

3번째 줄(`import static org.assertj.core.api.Assertions.assertThatThrownBy;`) 바로 아래에 추가한다.

```java
import static org.assertj.core.api.Assertions.tuple;
```

- [ ] **Step 5: `generateQuestion_relevantGoal_returnsQuestions` 테스트를 emoji/label 구조로 갱신**

133~158번째 줄(테스트 메서드 전체)을 아래로 교체한다.
...
```bash
git add server/src/main/java/com/chuseok22/elumserver/routine/application/dto/response/RoutineQuestionResponse.java \
...
git commit -m "feat: 일과 추가 질문 API 응답에 emoji/label 옵션 구조 반영"
```

---

### Task 3: Swagger 문서 문구 정리 — `RoutineControllerDocs`
...
73~83번째 줄(`@Operation(... summary = "AI 추가 질문 생성" ...)` 전체)을 아래로 교체한다.
...
## Self-Review 결과(이 계획 작성자가 직접 확인한 내역)

- **스펙 커버리지**: 설계서의 6개 섹션(옵션 데이터 모델/Gemini 스키마·프롬프트/fallback/방어적
  검증/Swagger 문서/테스트) 모두 Task 1~3에 대응됨. "client 반영 범위 밖" 항목은 이 계획에서
  아무 Task도 만들지 않음으로써 반영됨.
- **플레이스홀더 스캔**: "TBD"/"나중에" 류 문구 없음. 모든 Step에 실제 코드 또는 실행 가능한
  명령어를 포함시킴.
- **타입 일관성 확인**: `Option`(Task1, Draft) → `OptionResult`(Task1, 파이프라인) →
  `OptionItem`(Task2, API 응답) 세 이름이 계층마다 다르지만 각 계층 내부에서는 Task 전체에서
  동일하게 사용됨(다른 이름인 이유는 기존 계층 명명 규칙 — `QuestionItem`/`QuestionResultItem`도
  이미 계층마다 다른 이름을 쓰고 있었음). `RoutineService.toOptionItems()`의 파라미터 타입이
  Task 1에서 만든 `RoutineAiPipeline.RoutineQuestionResult.QuestionResultItem.OptionResult`와
  정확히 일치하는지 재확인함 — 일치.
