...
### Task 12: 루틴 단계에 카드 제목(title) 필드 추가
...
다루므로, 카드에 표시할 짧은 라벨과 소리 내어 읽어줄 문장을 분리해야 한다. 이 태스크는
계획 승인 이후 추가된 범위이며, 스펙 문서(`docs/superpowers/specs/2026-07-21-gemini-prompt-restructure-design.md`)에는
반영돼 있지 않다 — 사용자가 "계획에만 반영하고 진행"으로 확정했으므로 스펙 문서는 별도로
갱신하지 않는다.
...
| 항목 | 결정 |
|---|---|
| 새 필드 이름 | `title` — 단계별 카드에 굵게 표시되는 짧은 라벨(`RoutineStepDraft.StepDraft`, `RoutineAiPipeline.GeneratedStep`, `RoutineStep` 엔티티, `RoutineStepResponse`에서 전부 `title`로 통일). 루틴 전체 제목인 `Routine.title`/`RoutineResponse.title`과는 별개 필드로, JSON 구조상 `routine.title`(전체 제목) vs `routine.steps[].title`(단계별 짧은 라벨)로 중첩 위치가 달라 혼동되지 않는다 |
| `description`의 역할 | 필드명·존재는 그대로 유지하고, "아이에게 소리 내어 읽어줄 약간 더 자세한 문장"으로 의미를 명확히 한다 |
| 이미지 생성 장면 텍스트 | `description`을 계속 사용한다(기존 동작 유지, `title`로 바꾸지 않음) |
| DB 컬럼 | nullable로 추가하고, blank 검증은 애플리케이션 레벨(`parseDraft()`)에서만 한다(Step 5 참고) |
...
**Interfaces:**
- Produces: `RoutineStepDraft.StepDraft(Integer order, String title, String description)`(2개
  → 3개 인자로 변경), `RoutineAiPipeline.GeneratedStep(Integer order, String title, String
  description, String imagePath)`(3개 → 4개 인자로 변경), `RoutineStep.getTitle()/setTitle()`,
  `RoutineStepResponse.title()`.
...
- [ ] **Step 1: RoutineStepDraft.StepDraft에 title 추가 — 실패하는 테스트 먼저**
...
Expected: FAIL — `StepDraft`에 `title` 컴포넌트가 없어 컴파일 실패.
...
- [ ] **Step 2: GeminiTextClient.responseSchema()에 title 필드 추가**
...
- [ ] **Step 3: PromptDefaults — CREATE 프롬프트에 title/description 역할 분리 반영**
...
- [ ] **Step 4: PromptDefaults — REVISE 프롬프트에 title 보존 규칙 추가**
...
DB 컬럼을 nullable로 두기로 했으므로(Step 7), Gemini가 title 없이 응답해도 곧바로 500
...
`description`을 소스로 쓰므로(Step 열린 사항 참고), title만 바뀌고 description이 같으면
이미지는 재사용해도 된다.
...
"RoutineStepDraft.StepDraft(order, title, description)를 그대로 재사용한다"로 바꾼다.

- [ ] **Step 7: RoutineStep 엔티티에 title 컬럼 추가(nullable)**
...
- [ ] **Step 8: RoutineService.toStepEntities()에 title 매핑 추가**
...
- [ ] **Step 9: RoutineStepResponse에 title 노출**
...
`RoutineAiPipelineTest.java`에서 아래 4개 지점의 JSON 문자열에 `title`를 추가하고,
`generateForCreate_validResponse_returnsGeneratedStepsInArrayOrder`에는 title 검증
어서션도 추가한다. title 필드 없이 남겨두면 Step 5에서 추가한 검증 때문에 이 테스트들이
...
```java
    ...
    assertThat(result.steps().get(1).title()).isEqualTo("우산을 챙겨요");
```
...
git add server/src/main/java/com/chuseok22/elumserver/ai/core/RoutineStepDraft.java \
  ...
  server/src/test/java/com/chuseok22/elumserver/routine/infrastructure/ai/RoutineAiPipelineTest.java \
  server/src/test/java/com/chuseok22/elumserver/routine/application/service/RoutineServiceTest.java
git commit -m "feat: 루틴 단계에 카드 제목(title) 필드 추가, description은 읽어주기용 문장으로 역할 명확화"
```
